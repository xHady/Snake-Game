...
# ------------------- Configure Canvas ----------------------------
canvas = Canvas(
    root, width=800, height=800, bg="saddle brown", highlightbackground="saddle brown"
)
canvas.pack()
# ------------------------------------------------------------------
...
def on_press(event):  # handle Keyboard
    ...
    global MoveStarted
    if (
        (event.keycode == 37 or event.keycode == 65)
        and direction != 0
        and direction != 3
    ):
        # print("left")
        ...
        (event.keycode == 40 or event.keycode == 83)
        and direction != 1
        and direction != 2
    ):
        # print("Down")
        ...
        (event.keycode == 39 or event.keycode == 68)
        and direction != 3
        and direction != 0
    ):
        # print("Right")
        ...
        (event.keycode == 38 or event.keycode == 87)
        and direction != 2
        and direction != 1
    ):
        # print("Up")
...
def drawRect(RectPos, color):  # Draws A Rectangle
...
def addSnakeNode():  # simple Algorithm to add Node to Snake Tail
    ...
    lastTwoNodesXCoords = (
        lastNodeX - canvas.coords(SnakePosition[len(SnakePosition) - 2])[0] / cellSize
    )
    lastTwoNodesYCoords = (
        lastNodeY - canvas.coords(SnakePosition[len(SnakePosition) - 2])[1] / cellSize
    )
